...
-- Enable RLS on the view (by creating policies on underlying tables)
-- Admin policy for pickup_queue_view access
create policy if not exists "Admins can view pickup queue" on public.claims
for select using (
  exists (
...
create policy if not exists "Admins can view orders for pickup" on public.orders  
for select using (
  exists (
    select 1 from public.profiles p 
    where p.id = auth.uid() and p.is_admin = true
